...
# 建立auto框架
ae = sparse_ae_util.aebuilder(nodes)

# 设置部分参数
# 训练，迭代次数为200
ae = sparse_ae_util.aetrain(ae, x, 10000)
##############################
# step2 微调
...
# 开始进行神经网络训练，主要就是进行微调
aecomplete = sparse_ae_util.nntrain(aecomplete, x, y, 5000)
# 打印出最后一层的输出

result = pd.DataFrame(aecomplete.values[3])
writer = pd.ExcelWriter('C:/Users/7560/Desktop/山东化探数据库/result.xlsx') # 写入result文件
